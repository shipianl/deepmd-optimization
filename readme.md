...
$$
\mathcal{R}=\{{r}_{1}^{T}, \cdots, {r}_{i}^{T}, \cdots, {r}_{N}^{T}\}^{T}, \quad
{r}_{i}=\left(x_{i}, y_{i}, z_{i}\right) \tag{1}
$$

${r}_{i}$ 表示原子 $i$ 的三维笛卡尔坐标。将坐标矩阵 $\mathcal{R}$ 转换成局域坐标矩阵 $\{{\mathcal{R}}^{i}\}_{i=1}^{N}$：

$$
{\mathcal{R}}^{i}=\{{r}_{1 i}^{T}, \cdots, {r}_{j i}^{T}, \cdots, {r}_{N_{i}, i}^{T}\}^{T}, \quad
{r}_{j i}=\left(x_{j i}, y_{j i}, z_{j i}\right) \tag{2}
$$
...
$$
E=\sum_{i} E_{i}=\sum_{i} E\left(\mathcal{R}^{i}\right) \tag{4}
$$
...
$$
\{x_{j i}, y_{j i}, z_{j i}\} \mapsto\{s\left(r_{j i}\right), \hat{x}_{j i}, \hat{y}_{j i}, \hat{z}_{j i}\} \tag{5}
$$
...
$$
\mathcal{D}^{i}=\left(\mathcal{G}^{i 1}\right)^{T} \tilde{\mathcal{R}}^{i}\left(\tilde{\mathcal{R}}^{i}\right)^{T} \mathcal{G}^{i 2} \tag{7}
$$
...
$$
d_{k}^{\mathrm{out}}=\varphi\left(\sum_{k l} w_{k l} d_{l}^{\mathrm{in}}+ b_{k}\right) \tag{8}
$$
...
$$
L\left(p_{\epsilon}, p_{f}, p_{\xi}\right)=\frac{p_{\epsilon}}{N} \Delta \epsilon^{2}+\frac{p_{f}}{3 N} \sum_{i}\left|\Delta {F}_{i}\right|^{2}+\frac{p_{\xi}}{9 N}\|\Delta \xi\|^{2} \tag{9}
$$

其中 $\Delta \epsilon$、$\Delta {F}_{i}$ 和 $\Delta \xi$ 分别表示能量、力和维里的方均根误差（RMSE）。在训练过程中，前置因子 $p_{\epsilon}$、$p_{f}$ 和 $p_{\xi}$ 由下式决定：

$$
p(t)=p^{\operatorname{limit}}\left[1-\frac{r_{l}(t)}{r_{l}^{0}}\right]+p^{\operatorname{start}}\left[\frac{r_{l}(t)}{r_{l}^{0}}\right] \tag{10}
$$

其中 $r_{l}(t)$ 和 $r_{l}^{0}$ 分别表示在训练步数为 $t$ 和训练步数为 $0$ 时的学习率。$r_{l}(t)$ 的定义为：

$$
r_{l}(t)=r_{l}^{0} \times d_{r}^{\,t / d_{s}} \tag{11}
$$
